...
export default function Home() {
  ...
  useEffect(() => {
    const initializeWebApp = async () => {
      try {
        ...
        setWebApp(app);
        app.expand();
        if (app.initDataUnsafe.user) setUserData(app.initDataUnsafe.user);
  ...
  const handleShareContacts = async () => {
    ...
    setIsLoading(true);
    try {
      // This will be done through chat?
      ...
      const { status } = await axios.post(`${API_URL}/contacts`, contacts, { headers: { Authorization: webApp?.initData } });
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center p-24">
      ...
      <button onClick={handleShareContacts} className="mb-4 px-4 py-2 bg-green-500 text-white rounded hover:bg-green-600">
        Share Contacts
      </button>
          ...
          <div className="bg-white shadow rounded-lg">
            {contacts.map((contact) => (
              <div key={contact.id} className="p-4 border-b last:border-b-0">
                ...
                {contact.username && <p className="text-gray-500">@{contact.username}</p>}
              </div>
            ))}
          </div>
        ) : (
